...
import { faClipboardCheck, faFileContract, faFileInvoiceDollar, faGreaterThan, faReceipt } from '@fortawesome/free-solid-svg-icons';
                    ...
                    <div className='col-md-4'>
                        <ul className='proposals-list'>
                            {MiddleSectionTitleList.map((data, index) => (
                                <li
                                    key={index}
                                    className={`proposal-item ${selectedItem.slugs === data.slugs ? 'active' : ''}`}
                                    onClick={() => handleItemClick(data)}
                                >
                                    <h4><FontAwesomeIcon icon={data.icon} />&nbsp;&nbsp;{data.title}</h4>
                                    {selectedItem.slugs === data.slugs && <h5><FontAwesomeIcon icon={faGreaterThan} /></h5>}
                                </li>
                            ))}
                        </ul>
                    </div>
                    <div className='col-md-8 rounded' style={{ backgroundColor: '#fff5e8', paddingTop: '20px', paddingBottom: '10px' }}>
